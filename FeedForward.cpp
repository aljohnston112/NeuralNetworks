...
FeedForward::FeedForward(
	...
	int nOutputs,
	ActivationFunctions::ActivationFunction* a,
	ErrorFunctions::ErrorFunction* e,
	double learningRate
) {
	std::vector<std::unique_ptr<Perceptron>> firstLayer{};
	for(int i = 0; i < nInputs; i++) {
		firstLayer.emplace_back(std::make_unique<Perceptron>(nInputs, a, e));
	}
	...
	for(int j = 0; j < nHiddenLayerNeurons.size(); j++) {
		...
		for(int i = 0; i < nHiddenLayerNeurons[j]; i++) {
			hiddenLayer.emplace_back(std::make_unique<Perceptron>(nHiddenLayerNeurons[j], a, e));
		}
		neurons.emplace_back(std::move(hiddenLayer));
	}
	std::vector<std::unique_ptr<Perceptron>> lastLayer{};
	for(int i = 0; i < nOutputs; i++) {
		lastLayer.emplace_back(std::make_unique<Perceptron>(nInputs, a, e));
	}
